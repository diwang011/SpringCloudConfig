...
import com.example.jpa.CScheduleTriggerRepository;
...
public class ScheduleTriggerServiceImpl implements ScheduleTriggerService
{
    ...
    @Override
    @Scheduled(cron = "0 29 15 * * ?") // 每天晚上11点调用这个方法来更新quartz中的任务
    public void refreshTrigger()
    {
        try
        {
            // 查询出数据库中所有的定时任务
            List<CScheduleTrigger> jobList = (List<CScheduleTrigger>) triggerRepository.findAll();
            if (jobList != null)
            {
